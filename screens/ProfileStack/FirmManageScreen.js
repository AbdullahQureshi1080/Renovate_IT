...
export default function FirmManageScreen({navigation,route}) {
  ...
  const [value,setValue] = useState([]);
    ...
    const handleUpdateNote = async (noteId)=>{
      ...
      console.log("Updating Note",email,noteId,firmId,text,images,documents)
      // console.log("Add Note")
      if(text =="" ){
        Alert.alert("Cannot have empty note")
        return;
    ...
    for (var i = 0; i < documents.length; i++) {
      ...
      setIsVisible2(false);
      setIsVisible4(false);
       const result = await updateNoteApi.request(email,noteId,firmId,note,arrImages,arrDocuments); 
      // // const result = await noteApi.request(firmId,note,email,arrImages,arrDocuments);     
      if(!result.ok){
    ...
    return (
      <MenuProvider>
        ...
        updateName="Update"
        onPressUpdate={()=>{setIsVisible4(true), setDocuments(modalData?.documents), setImages(modalData.images), setText(modalData.note)}}
